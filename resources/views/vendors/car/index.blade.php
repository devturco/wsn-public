  ...
  @if ($current_package != '[]')
    @if (vendorTotalAddedCar() > $current_package->number_of_car_add)
      ...
      <div class="mt-2 mb-4">
        <div class="alert alert-danger text-dark">
          <ul>
            <li>{{ __('Você adicionou um total ') . vendorTotalAddedCar() }} {{ __(' cars.') }}</li>
            <li>{{ __('Seu pacote atual suporta') . ' ' . $current_package->number_of_car_add . ' cars.' }} </li>
            <li>{{ __('Você tem que remover ') }}
    ...
    @if (vendorTotalFeaturedCar() > $current_package->number_of_car_featured)
      ...
      <div class="mt-2 mb-4">
        <div class="alert alert-danger text-dark">
          <ul>
            <li>{{ __('Você tem total  ') . vendorTotalFeaturedCar() . ' featured cars.' }}</li>
            <li>
              {{ __('Com seu pacote atual você pode apresentar ') . $current_package->number_of_car_featured . __(' cars.') }}
            </li>
            <li>{{ __('Seus carros foram removidos da seção de carros em destaque do nosso site.') }}
            </li>
            <li>{{ __('Você tem que descaracterizar ') }}
              {{ vendorTotalFeaturedCar() - $current_package->number_of_car_featured . __(' carros para mostrar seus carros na seção de carros em destaque do nosso site.') }}
            </li>
          </ul>
  ...
  @else
    ...
    @if ($pendingPackage)
      <div class="alert alert-warning text-dark">
        {{ __('Você solicitou um pacote que precisa de uma ação (aprovação/rejeição) do administrador. Você será notificado por e-mail assim que uma ação for tomada.') }}
      </div>
      <div class="alert alert-warning text-dark">
    ...
    <div class="col-md-12">
      ...
      <div class="card">
        <div class="card-header">
          <div class="row">
            <div class="col-lg-4">
              <div class="card-title d-inline-block">{{ __('Carros') }}</div>
            </div>
            ...
            <div class="col-lg-4 offset-lg-1 mt-2 mt-lg-0">
              <a href="{{ route('vendor.cars_management.create_car') }}" class="btn btn-primary btn-sm float-right"><i
                  class="fas fa-plus"></i> {{ __('Cadastrar carro') }}</a>
        ...
        <div class="card-body">
          <div class="row">
            <div class="col-lg-12">
              @if (count($cars) == 0)
                <h3 class="text-center">{{ __('Nenhum carro encontrado!') }}</h3>
              @else
