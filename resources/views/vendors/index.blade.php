...
@section('content')
  <div class="mt-2 mb-4">
    <h2 class="pb-2">{{ __('Olá,') }} {{ Auth::guard('vendor')->user()->username . '!' }}</h2>
  </div>
  ...
  @if (is_null($package))
    ...
    @else
      <div class="alert alert-warning text-dark">
        {{ __('Sua assinatura expirou. Por favor, compre um novo pacote/amplie o pacote atual.') }}
      </div>
    @endif
  @else
  ...
  <div class="row dashboard-items">
    ...
    <div class="col-lg-6">
      <div class="card">
        <div class="card-header">
          <div class="card-title">{{ __('Cadastros de carros/mês') }} ({{ date('Y') }})</div>
        </div>
    ...
    <div class="col-lg-6">
      <div class="card">
        <div class="card-header">
          <div class="card-title">{{ __('Visitas por mês') }} ({{ date('Y') }})</div>
        </div>
